...
void pv_debug(Search *search, const Move *bestmove, FILE *f)
{
	...
	*board = *search->board;
	if (search->height == 1)
	{ // hack to call this function from height 1 or 0
	...
	*board = *search->board;
	...
	while (search_depth > 0 && x != NOMOVE)
	{
		...
		else if (hash_get(search->hash_table, board, hash_code, hash_data))
		{
			...
			break;
		if (hash_data->depth < search_depth || hash_data->selectivity < search->selectivity || hash_data->lower != hash_data->upper)
			return false;
		if (x == NOMOVE && !board_is_game_over(board))
			return false;
	}
	return true;
}
	...
	*board = *init_board;
	...
	if (bound->lower > bound->upper)
	{
		if (result->score < beta)
			...
			bound->upper = search->stability_bound.upper;
		if (result->score > alpha)
			bound->lower = result->score;
		else
			bound->lower = search->stability_bound.lower;
	}
	...
	while (x != NOMOVE)
	{
		...
		if (board_check_move(board, move))
		{
			board_update(board, move);
			--expected_depth;
			tmp = expected_bound.upper;
			expected_bound.upper = -expected_bound.lower;
			expected_bound.lower = -tmp;
			fail_low = !fail_low;
			line_push(result->pv, move->x);

			hash_code = board_get_hash_code(board);
			if ((hash_get(search->pv_table, board, hash_code, hash_data) || hash_get(search->hash_table, board, hash_code, hash_data)) && (hash_data->depth >= expected_depth && hash_data->selectivity >= expected_selectivity) && (hash_data->upper <= expected_bound.upper && hash_data->lower >= expected_bound.lower))
			{
				x = hash_data->move[0];
			}
			else
				x = NOMOVE;
			if (guess_pv && x == NOMOVE && fail_low)
				x = guess_move(search, board);
		}
		else
			x = NOMOVE;
	}
	...
	if (log_is_open(search_log))
	{
		...
		if (result->time > 0)
		{
			log_print(search_log, "speed = %9.0f Nps", 1000.0 * result->n_nodes / result->time);
		}
	...
	if (has_changed && options.noise <= depth && search->options.verbosity == 3)
...
{
	if (score < search->stability_bound.lower)
		score = search->stability_bound.lower;
	if (score > search->stability_bound.upper)
		score = search->stability_bound.upper;
	return score;
}
...
static int search_route_PVS(Search *search, int alpha, int beta, const int depth, Node *node)
{
	...
	{
		if (depth == 0)
			score = search_solve_0(search);
		else
			score = PVS_midgame(search, alpha, beta, depth, node);
	}
	else
	{
		if (depth == 0)
			score = search_eval_0(search);
		else if (depth == 1)
			score = search_eval_1(search, alpha, beta);
		else if (depth == 2)
			score = search_eval_2(search, alpha, beta);
		else
			score = PVS_midgame(search, alpha, beta, depth, node);
	}
 ...
 * @return A score, as a disc difference.
 */
EMSCRIPTEN_KEEPALIVE int PVS_root(Search *search, const int alpha, const int beta, const int depth)
{
	HashData hash_data[1];
	...
	if (search->options.verbosity == 4)
		printf("PVS_root [%d, %d], %d@%d%%\n", alpha, beta, depth, selectivity_table[search->selectivity].percent);
	...
	else
	{
		...
		if ((move = node_first_move(node, movelist)))
		{
			...
			while ((move = node_next_move(node)))
			{
				...
				if (search->options.verbosity == 4)
					pv_debug(search, move, stdout);
				if (search_time(search) > search->time.maxi && node->bestscore > alpha)
	...
	if (!search->stop)
	{
		...
		if (depth < search->options.multipv_depth)
			...
			movelist_sort_cost(movelist, hash_data);
		...
		if (movelist->n_moves == get_mobility(board->player, board->opponent))
		{
			...
			if (search->options.guess_pv)
				hash_force(search->pv_table, board, hash_code, depth, search->selectivity, last_bit(cost), alpha, beta, node->bestscore, node->bestmove);
			else
				hash_store(search->pv_table, board, hash_code, depth, search->selectivity, last_bit(cost), alpha, beta, node->bestscore, node->bestmove);
		}
...
int aspiration_search(Search *search, int alpha, int beta, const int depth, int score)
{
	...
	if (width < 1)
		width = 1;
	if ((width & 1) && depth == search->n_empties)
		++width;
	...
	for (i = 0; i < 10; ++i)
	{
		...
		{ // otherwise iterate search with small windows until the score is bounded by the window, or a cut
		...
		if (is_depth_solving(depth, search->n_empties) && ((alpha < score && score < beta) || (score == alpha && score == options.alpha) || (score == beta && score == options.beta)) && !is_pv_ok(search, search->result->move, depth))
		{
			...
			{
				printf("DEBUG: Wrong PV: ");
				pv_debug(search, movelist_first(search->movelist), stdout);
				putchar('\n');
				fflush(stdout);
				if (log_is_open(engine_log))
				{
					fprintf(engine_log->f, "DEBUG: Wrong PV: ");
					pv_debug(search, movelist_first(search->movelist), engine_log->f);
					putc('\n', engine_log->f);
					fflush(engine_log->f);
				}
			}
	...
	if (!search->stop)
		record_best_move(search, search->board, movelist_first(search->movelist), alpha, beta, depth);
...
void iterative_deepening(Search *search, int alpha, int beta)
{
	...
	if (USE_PROBCUT && search->options.depth > 10)
		search->selectivity = 0;
	else
		search->selectivity = NO_SELECTIVITY;

	old_depth = 0;
	old_selectivity = search->selectivity;
	...
	if (search->selectivity > search->options.selectivity)
		search->selectivity = search->options.selectivity;

	if (start > search->options.depth)
		start = search->options.depth;
	if (start > search->n_empties)
		start = search->n_empties;
	if (start < search->n_empties)
	{
		if ((start & 1) != (end & 1))
			++start;
	...
	tmp_selectivity = search->selectivity;
	search->selectivity = old_selectivity;
	if (!movelist_is_empty(movelist))
	{
		if (end == 0)
	...
	for (search->depth = start; search->depth < end; search->depth += 2)
	{
		...
		if (!search_continue(search))
			return;
		if (abs(score) >= SCORE_MAX - 1 && search->depth > end - ITERATIVE_MIN_EMPTIES && search->options.depth >= search->n_empties)
			break;
	}
	search->depth = end;

	// switch to endgame
	if (search->options.depth >= search->n_empties)
	...
	for (; search->selectivity <= search->options.selectivity; ++search->selectivity)
	{
		if (search->depth == search->n_empties &&
				((search->depth < 21 && search->selectivity >= 1) || (search->depth < 24 && search->selectivity >= 2) || (search->depth < 27 && search->selectivity >= 3) || (search->depth < 30 && search->selectivity >= 4) || (has_time && search->depth < 30 && search->selectivity >= 2) || (abs(score) >= SCORE_MAX)))
		{ // jump to exact endgame (to solve faster) ?
			search->selectivity = search->options.selectivity;
		}
		if (search->selectivity == search->options.selectivity)
			search_adjust_time(search, true);
...
void *search_run(void *v)
{
	...
	{
		if (search->options.verbosity == 1 || options.noise > search->result->depth)
			search->observer(search->result);
		if (search->stop == STOP_TIMEOUT)
		{
			info("[Search out of time]\n");
		}
		else if (search->stop == STOP_ON_DEMAND)
		{
			info("[Search stopped on user demand]\n");
		}
		else if (search->stop == STOP_PONDERING)
		{
			info("[Pondering stopped]\n");
		}
		else if (search->stop == RUNNING)
		{
	...
	if (log_is_open(search_log))
	{
		...
		if (search->stop == STOP_TIMEOUT)
			log_print(search_log, "out of time");
		else if (search->stop == STOP_ON_DEMAND)
			log_print(search_log, "stopped on user demand");
		else if (search->stop == STOP_PONDERING)
			log_print(search_log, "stop pondering");
		else if (search->stop == STOP_PARALLEL_SEARCH)
			log_print(search_log, "### BUG: stop parallel search reached root! ###");
		else if (search->stop == RUNNING)
			log_print(search_log, "completed");
		else
			log_print(search_log, "### BUG: unkwown stop condition %d ###", search->stop);
		log_print(search_log, " ***\n\n");
		unlock(search_log);
	}
